...
export function AutomationsClient({ workflows }: { workflows: N8nWorkflow[] }) {
    ...
    const selected = selectedWorkflow ? workflows.find((w) => w.id === selectedWorkflow) : null;
                ...
                <div className="grid grid-cols-2 gap-4 lg:grid-cols-4 mb-6">
                    <Card className="card-elevated border-transparent bg-gradient-to-br from-white to-zinc-50/50">
                        <CardContent className="pt-6 relative z-10">
                            ...
                            <p className="text-3xl text-foreground mt-2">{workflows.length}</p>
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
                            {workflows.map((workflow) => {
                                ...
                                const TriggerIcon = workflow.triggerType === "schedule" 
                                    ? Calendar 
                                        ...
                                        <CardContent className="pt-5 pb-4">
                                            ...
                                            <div className="flex gap-1 mt-2">
                                                {workflow.tags.map((tag) => (
                                                    <span key={tag} className="px-2 py-0.5 bg-muted border border-border/60 rounded-md text-[11px] text-foreground/70 font-medium">
                                                        {tag}
                                                    </span>
                                                ))}
                                            </div>
