...
export function AutomationsClient({ workflows }: { workflows: N8nWorkflow[] }) {
      ...
      <div className="">
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-sm text-muted-foreground">
                  Automated workflows powered by n8n — trigger via Telegram,
                  schedule, or webhook
                </p>
              </div>
                ...
                return (
                  ...
                  >
                    <CardContent className="pt-5 pb-4">
                      ...
                      {/* Description */}
                      <p className="text-[13px] font-medium text-muted-foreground mb-4 line-clamp-2 leading-relaxed">
                      ...
                      <div className="flex items-center gap-1 mb-3 overflow-x-auto">
                        {workflow.nodes.map((node, i) => (
                          ...
                          >
                            <span className="px-2.5 py-1 bg-muted border border-border/60 rounded-md text-[11px] text-foreground/80 font-medium">
                              {node}
                            </span>
                      ...
                      <div className="flex gap-1 mt-2">
                        {workflow.tags.map((tag) => (
                          <span
                            key={tag}
                            className="px-2 py-0.5 bg-muted border border-border/60 rounded-md text-[11px] text-foreground/70 font-medium"
                          >
                            {tag}
                          </span>
                        ))}
                    ...
                    <TableBody>
                      {selected.recentExecutions.map((exec) => (
                        <TableRow key={exec.id} className="group">
                          <TableCell>
                            <span
                              className={`flex items-center gap-1.5 text-xs font-semibold ${execStatusStyle[exec.status]}`}
                            >
                              {exec.status === "success" ? (
                          ...
                          <TableCell className="text-right text-muted-foreground text-[13px] font-medium">
                            {exec.duration}
                          </TableCell>
                          <TableCell className="text-right text-muted-foreground text-[13px] font-medium">
                    ...
                    {workflows.flatMap((wf) =>
                      wf.recentExecutions.map((exec) => (
                        <TableRow key={exec.id} className="group">
                          ...
                          <TableCell>
                            <span
                              className={`flex items-center gap-1 text-xs font-semibold ${execStatusStyle[exec.status]}`}
                            >
                              {exec.status === "success" ? (
        ...
        {activeTab === "editor" && (
          <div className="space-y-6">
                ...
                <div className="flex flex-col items-center text-center gap-5">
                  ...
                  <div>
                    ...
                    <p className="text-sm font-medium text-muted-foreground mt-2 max-w-md mx-auto leading-relaxed">
                      Create and manage your automation workflows with the full
                      n8n visual editor
                    </p>
                  </div>
                  <Button
            ...
            {/* Quick access cards */}
            <div>
              <h4 className="text-[15px] mb-4">
                Quick access — jump to workflow
              </h4>
              <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
                {[
                  ...
                  <Card
                    ...
                    onClick={() =>
                      window.open(`${n8nUrl}/home/workflows`, "_blank")
                    }
                  >
              ...
              <CardContent>
                <div className="grid grid-cols-1 gap-2 sm:grid-cols-2">
                  {[
                    ...
                    <div
                      key={ep.label}
                      className="flex items-center p-2.5 rounded-lg bg-muted/50 border"
                    >
          ...
          <div className="space-y-6 max-w-2xl">
              ...
              <CardContent className="space-y-5 pt-6">
                <div className="space-y-2.5">
                  ...
                  <p className="text-[11px] font-medium text-muted-foreground pl-1">
                    The URL of your n8n instance (must be accessible from this
                    browser)
                  </p>
                </div>
                <div className="space-y-2.5">
                ...
                <Button
                  ...
                  className="w-full gap-1.5 h-9 bg-primary text-primary-foreground hover:bg-primary/95 shadow-primary/20"
                >
                  {isConnected ? (
                    <>
                      <XCircle className="h-4 w-4" />
            ...
            <Card className="card-elevated border-transparent">
              <CardHeader className="border-b border-border/40 bg-muted/50">
                ...
                <CardDescription className="font-medium">
                  Use these URLs in your n8n workflows to connect with Digital
                  Workforce
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-3 pt-6">
                {[
                  {
                    label: "AI Planner Webhook",
                    url: "https://n8n.tulie.app/webhook/planner",
                  },
                  {
                    label: "Content Creator Webhook",
                    url: "https://n8n.tulie.app/webhook/content",
                  },
                  {
                    label: "Design Specialist Webhook",
                    url: "https://n8n.tulie.app/webhook/design",
                  },
                  {
                    label: "Digital Ads Webhook",
                    url: "https://n8n.tulie.app/webhook/ads",
                  },
                ].map((hook) => (
                  <div
                    key={hook.label}
