...
const NODE_CATALOG: NodeTemplate[] = [
    // Triggers
    { type: "telegram_trigger", label: "Telegram Trigger", category: "Triggers", icon: MessageSquare, color: "#0088cc", description: "Listen for Telegram messages or commands", inputs: 0, outputs: 1, defaultConfig: { command: "/task", botToken: "" } },
    { type: "schedule_trigger", label: "Schedule", category: "Triggers", icon: Clock, color: "#6B7280", description: "Run on a cron schedule", inputs: 0, outputs: 1, defaultConfig: { cron: "0 9 * * *", timezone: "Asia/Ho_Chi_Minh" } },
    { type: "webhook_trigger", label: "Webhook", category: "Triggers", icon: Webhook, color: "#8B5CF6", description: "Trigger via HTTP webhook", inputs: 0, outputs: 1, defaultConfig: { method: "POST", path: "/webhook" } },

    // AI
    { type: "openai", label: "OpenAI GPT-4o", category: "AI", icon: Brain, color: "#10A37F", description: "Generate text with GPT-4o", inputs: 1, outputs: 1, defaultConfig: { model: "gpt-4o", temperature: "0.7", prompt: "" } },
    { type: "openai_mini", label: "GPT-4o Mini", category: "AI", icon: Brain, color: "#10A37F", description: "Lightweight AI for simple tasks", inputs: 1, outputs: 1, defaultConfig: { model: "gpt-4o-mini", temperature: "0.5", prompt: "" } },

    // Actions
    { type: "facebook_post", label: "Facebook Post", category: "Actions", icon: Facebook, color: "#1877F2", description: "Post to a Facebook Page", inputs: 1, outputs: 1, defaultConfig: { pageId: "", accessToken: "" } },
    { type: "telegram_send", label: "Telegram Send", category: "Actions", icon: MessageSquare, color: "#0088cc", description: "Send message back to Telegram", inputs: 1, outputs: 1, defaultConfig: { chatId: "", message: "" } },
    { type: "send_email", label: "Send Email", category: "Actions", icon: Mail, color: "#EA4335", description: "Send email via SMTP or SendGrid", inputs: 1, outputs: 1, defaultConfig: { to: "", subject: "", body: "" } },
    { type: "http_request", label: "HTTP Request", category: "Actions", icon: Globe, color: "#F59E0B", description: "Make an HTTP API call", inputs: 1, outputs: 1, defaultConfig: { url: "", method: "GET" } },

    // Data
    { type: "supabase", label: "Supabase", category: "Data", icon: Database, color: "#3ECF8E", description: "Read/write to Supabase database", inputs: 1, outputs: 1, defaultConfig: { table: "", operation: "select" } },
    { type: "google_sheets", label: "Google Sheets", category: "Data", icon: FileText, color: "#34A853", description: "Read/write Google Sheets", inputs: 1, outputs: 1, defaultConfig: { spreadsheetId: "", range: "A1:Z" } },

    // Logic
    { type: "if_condition", label: "IF", category: "Logic", icon: GitBranch, color: "#F97316", description: "Branch based on condition", inputs: 1, outputs: 2, defaultConfig: { condition: "", trueLabel: "True", falseLabel: "False" } },
    { type: "filter", label: "Filter", category: "Logic", icon: Filter, color: "#6366F1", description: "Filter data by condition", inputs: 1, outputs: 1, defaultConfig: { field: "", operator: "equals", value: "" } },
];
...
const defaultNodes: WorkflowNode[] = [
    ...
    },
    {
        ...
        x: 370,
        y: 200,
        config: { model: "gpt-4o", temperature: "0.7", prompt: "Write a Facebook post about: {{$json.message}}" },
...
const defaultConnections: Connection[] = [
    { id: "conn-1", fromNodeId: "node-1", fromPortId: "node-1-out-0", toNodeId: "node-2", toPortId: "node-2-in-0" },
    { id: "conn-2", fromNodeId: "node-2", fromPortId: "node-2-out-0", toNodeId: "node-3", toPortId: "node-3-in-0" },
    { id: "conn-3", fromNodeId: "node-2", fromPortId: "node-2-out-0", toNodeId: "node-4", toPortId: "node-4-in-0" },
];
...
export function WorkflowEditor({ workflowName = "Telegram → AI → Facebook Post" }: WorkflowEditorProps) {
    ...
    const getConnectionPath = (conn: Connection): string => {
        ...
        const y1 = fromNode.y + NODE_H / 2 + (fromPortIndex - (fromNode.outputs.length - 1) / 2) * 20;
        const x2 = toNode.x;
        const y2 = toNode.y + NODE_H / 2 + (toPortIndex - (toNode.inputs.length - 1) / 2) * 20;
                ...
                {showPalette && (
                    <div className="w-64 border-r border-border bg-white overflow-y-auto shrink-0">
                        ...
                        <div className="p-2 space-y-1">
                            {NODE_CATALOG.filter((n) => n.category === paletteCategory).map((template) => {
