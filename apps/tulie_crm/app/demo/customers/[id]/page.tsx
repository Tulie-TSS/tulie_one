    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex flex-col md:flex-row md:items-center gap-4 md:gap-6">
                <div className="h-16 w-16 rounded-md bg-muted dark:bg-zinc-800 flex items-center justify-center shrink-0">
                    <span className="text-2xl font-black text-muted-foreground dark:text-zinc-300">{customer.name.split(' ').slice(-1)[0][0]}</span>
                </div>
                <div className="flex-1">
                    <h1 className="text-2xl font-bold text-foreground dark:text-zinc-50">{customer.name}</h1>
                    <p className="text-sm text-muted-foreground">{customer.company}</p>
                    <div className="flex flex-wrap gap-2 mt-2">
                        <Badge className={`text-[10px] font-semibold border-none px-2.5 py-1 rounded-lg ${customer.type === 'Doanh nghiệp' ? 'bg-violet-100 text-violet-700 dark:bg-violet-900/30 dark:text-violet-300' : 'bg-muted text-zinc-700 dark:bg-zinc-800 dark:text-zinc-300'}`}>
                            {customer.type}
                        </Badge>
                        <Badge className={`text-[10px] font-semibold border-none px-2.5 py-1 rounded-lg ${customer.status === 'active' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300' : 'bg-muted text-muted-foreground dark:bg-zinc-800 dark:text-muted-foreground'}`}>
                ...
                <div className="text-right">
                    ...
                    <p className="text-2xl font-bold text-foreground dark:text-zinc-50">{formatCurrency(customer.totalValue)}</p>
                </div>
            </div>

            {/* Contact */}
            <div className="grid gap-3 md:grid-cols-3">
                <div className="flex items-center gap-3 bg-white dark:bg-zinc-900 border border-border rounded-md p-4">
            ...
            <div className="grid gap-6 lg:grid-cols-2">
                ...
                <Card className="rounded-md border-border">
                    <CardHeader>
                        <CardTitle className="text-lg font-bold">Dự án</CardTitle>
                    </CardHeader>
                    <CardContent className="space-y-3">
                        ...
                        {customerProjects.map(p => (
                            <Link key={p.id} href={`/demo/projects/${p.id}`} className="block">
                                <div className="flex items-center gap-4 py-2 hover:bg-muted dark:hover:bg-zinc-800/50 rounded-lg px-2 -mx-2 transition-colors">
                                    <div className="flex-1 min-w-0">
                                        <p className="text-sm font-semibold text-foreground dark:text-zinc-50 truncate">{p.name}</p>
                                        <p className="text-xs text-muted-foreground">{p.designer}</p>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <div className="w-16 h-1.5 bg-muted dark:bg-zinc-700 rounded-full overflow-hidden">
            ...
            {customerInvoices.length > 0 && (
                <Card className="rounded-md border-border">
                    <CardHeader>
                        <CardTitle className="text-lg font-bold">Hóa đơn</CardTitle>
                    </CardHeader>
                    <CardContent>
                        <div className="divide-y divide-border/50">
                            {customerInvoices.map(inv => (
                                <Link key={inv.id} href={`/demo/invoices/${inv.id}`} className="block">
                                    <div className="flex items-center gap-4 py-3 hover:bg-muted dark:hover:bg-zinc-800/50 rounded-lg px-2 -mx-2 transition-colors">
                                        <div className="flex-1 min-w-0">
                                            <p className="text-sm font-semibold text-foreground dark:text-zinc-50">{inv.description}</p>
