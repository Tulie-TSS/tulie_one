        ...
        <div className="space-y-6">
            <div className="flex items-center gap-3">
                <div className="h-12 w-12 rounded-md bg-muted dark:bg-zinc-800 flex items-center justify-center shadow-sm border border-border">
                    <Users className="h-6 w-6 text-foreground dark:text-zinc-100" />
                </div>
                <div>
                    <h1 className="text-3xl font-bold text-foreground dark:text-zinc-50">Khách hàng</h1>
                ...
                <div className="divide-y divide-border/50">
                    {CUSTOMERS.map((c) => (
                        <Link key={c.id} href={`/demo/customers/${c.id}`} className="block">
                            <div className="grid grid-cols-1 md:grid-cols-12 gap-2 md:gap-4 px-5 py-4 hover:bg-muted/50 dark:hover:bg-zinc-800/50 transition-colors items-center cursor-pointer">
                                <div className="col-span-1">
                                    <span className="text-xs font-mono font-semibold text-muted-foreground">{c.id}</span>
                                </div>
                                <div className="col-span-3">
                                    <p className="text-sm font-semibold text-foreground dark:text-zinc-50">{c.name}</p>
                                ...
                                <div className="col-span-2">
                                    <Badge className={`text-[10px] font-semibold border-none px-2.5 py-1 rounded-lg ${c.type === 'Doanh nghiệp' ? 'bg-violet-100 text-violet-700 dark:bg-violet-900/30 dark:text-violet-300' : 'bg-muted text-zinc-700 dark:bg-zinc-800 dark:text-zinc-300'}`}>
                                        {c.type}
                                    </Badge>
                                </div>
                                <div className="col-span-2">
                                    <p className="text-sm font-bold text-foreground dark:text-zinc-50">{formatCurrency(c.totalValue)}</p>
                                </div>
                                <div className="col-span-2">
                                    <Badge className={`text-[10px] font-semibold border-none px-2.5 py-1 rounded-lg ${c.status === 'active' ? 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300' : 'bg-muted text-muted-foreground dark:bg-zinc-800 dark:text-muted-foreground'}`}>
