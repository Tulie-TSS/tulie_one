  ...
  return (
    <div className="min-h-screen bg-muted/50 font-sans text-foreground pb-20 selection:bg-black selection:text-white">
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 mt-6 sm:mt-10 space-y-6">
        {/* Info */}
        <div className="bg-white rounded-md border border-border p-4 sm:p-5 shadow-sm">
        ...
        <div className="grid gap-6 sm:grid-cols-2">
          {PRINT_SIZES.map(size => (
            <div key={size.id} className="bg-white rounded-md border border-border overflow-hidden shadow-sm">
              <div className="p-4 sm:p-5 border-b border-border">
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="text-[15px] font-bold text-foreground">{size.name}</h3>
                    <p className="text-xs text-muted-foreground font-medium mt-0.5">{size.desc}</p>
                  </div>
                  <span className="text-xs font-bold text-muted-foreground bg-muted px-2.5 py-1 rounded-full">{size.photos}</span>
                </div>
              </div>
        ...
        <div className="text-center py-8">
          <a 
            href="/anhthe" 
            className="group relative inline-flex flex-col items-center justify-center rounded-md font-bold h-auto py-4 px-12 bg-gradient-to-r from-zinc-900 via-zinc-800 to-zinc-900 hover:from-zinc-800 hover:via-zinc-700 hover:to-zinc-800 text-white shadow-xl shadow-black/20 hover:shadow-2xl hover:shadow-black/30 transition-all duration-300"
          >
            <span className="absolute -inset-1 rounded-md bg-gradient-to-r from-zinc-400/20 via-zinc-300/10 to-zinc-400/20 opacity-0 group-hover:opacity-100 transition-opacity blur-sm" />
            <span className="relative flex items-center gap-2 text-[15px]">
              ✦ Đặt ảnh thẻ ngay
            </span>
            <span className="relative text-[11px] font-medium text-muted-foreground mt-0.5">Chỉ từ 79.000đ — Giao tận nhà</span>
          </a>
        </div>
      </main>
    </div>
