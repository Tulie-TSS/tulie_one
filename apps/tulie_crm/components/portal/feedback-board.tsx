...
function ImageLightbox({ src, alt, onClose }: { src: string; alt: string; onClose: () => void }) {
    return (
        ...
        >
            <button
                onClick={onClose}
                className="absolute top-4 right-4 w-10 h-10 rounded-full bg-background/10 hover:bg-background/20 flex items-center justify-center text-primary-foreground transition-colors"
            >
                <X className="w-5 h-5" />
            </button>
            <img
            ...
            {showForm && (
                <div className="p-6 bg-muted border-b border-border">
                    <div className="max-w-4xl space-y-5 bg-background p-6 rounded-md border border-border/60 shadow-sm">
                        ...
                        <div>
                            <label className="text-xs font-bold text-foreground/80 mb-2 block uppercase tracking-wider">Tiêu đề yêu cầu <span className="text-rose-500">*</span></label>
                            <Input
                                value={newTitle}
                        ...
                        <div className="flex flex-col sm:flex-row gap-5 items-start sm:items-center justify-between pt-2">
                            <div className="flex items-center gap-3">
                                <span className="text-xs font-bold text-foreground/80 uppercase tracking-wider mr-1">Mức độ:</span>
                                {Object.entries(PRIORITY_CONFIG).map(([key, config]) => (
                                    <button
                                        ...
                                        className={cn(
                                            ...
                                            newPriority === key
                                                ? "bg-primary text-primary-foreground border-zinc-900 shadow-sm"
            ...
            <div className="border-t border-border">
                {isLoading ? (
                    ...
                    <div className="py-24 flex flex-col items-center justify-center text-center bg-muted/50">
                        ...
                        <Button variant="outline" onClick={() => setShowForm(true)} className="mt-6 bg-background shadow-sm font-semibold">
                            Tạo tác vụ đầu tiên
                        </Button>
                    </div>
                ) : (
                                            ...
                                            {isExpanded && (
                                                <TableRow className="border-b-zinc-200 bg-muted/50 hover:bg-muted/50">
                                                    <TableCell colSpan={6} className="p-0">
                                                        <div className="px-6 py-6 border-l-2 border-l-primary/30 ml-[25px] mb-4 mt-2 bg-background rounded-r-xl shadow-sm mr-6 border-y border-r border-border/60 overflow-hidden">
                                                                    ...
                                                                    <div className="flex items-start justify-between gap-4 mb-3">
                                                                        ...
                                                                        {isAdmin && editItemId !== item.id && (
                                                                            <div className="flex items-center gap-2">
                                                                                ...
                                                                                }} className="h-7 text-xs px-2 shadow-sm bg-background hover:bg-muted border-border text-muted-foreground font-semibold hover:text-foreground transition-colors">
                                                                                    <Edit className="w-3 h-3 mr-1.5" /> Sửa
                                                                                </Button>
                                                                                <Button variant="outline" size="sm" onClick={(e) => { e.stopPropagation(); handleDelete(item.id); }} disabled={isDeleting === item.id} className="h-7 text-xs px-2 shadow-sm bg-background hover:bg-rose-50 hover:text-rose-600 hover:border-rose-200 border-border transition-colors text-muted-foreground font-semibold">
