    ...
    return [
        {
            ...
            content: 'Website được phát triển thủ công trên nền tảng Next.js và Payload CMS, không sử dụng template có sẵn. Bên B dựng demo trực tiếp trên trình duyệt để hai bên chốt định hướng trước khi lập trình hoàn chỉnh.'
        },
        {
            key: 'scope_of_work',
            label: 'Phạm vi công việc',
            content: 'Bên B thực hiện các hạng mục đã ghi tại Bảng giá: demo giao diện cho các page template đã chốt; lập trình Frontend Next.js/React/TypeScript responsive; Backend, API, form và lead capture; Payload CMS quản trị nội dung; đa ngôn ngữ, SEO kỹ thuật; kiểm thử, đào tạo và go-live. Số lượng trang, ngôn ngữ, tích hợp và tính năng áp dụng đúng Bảng giá.'
        },
        {
            key: 'methodology',
            label: 'Phương pháp thực hiện',
            content: 'Áp dụng Design-Driven Development: duyệt prototype trực quan trước khi lập trình. Kiến trúc Headless CMS tách Frontend và Backend để tối ưu hiệu năng, SEO và khả năng mở rộng. Yêu cầu mới ngoài Bảng giá được đánh giá như phát sinh.'
        },
        {
            key: 'deliverables',
            label: 'Sản phẩm bàn giao',
            content: 'Bàn giao website vận hành trên môi trường đã thống nhất, mã nguồn Frontend/Backend theo điều kiện thanh toán của Hợp đồng, hệ thống CMS, tài khoản quản trị, tài liệu hướng dẫn sử dụng và tài liệu kỹ thuật. Nội dung, dữ liệu hoặc dịch vụ bên thứ ba không ghi tại Bảng giá không thuộc phạm vi mặc định.'
        },
        {
            key: 'timeline',
            label: 'Lộ trình triển khai tham khảo',
            content: 'Giai đoạn dự kiến: chốt demo giao diện; phát triển Frontend; phát triển Backend/CMS và tích hợp; SEO, đa ngôn ngữ và chuyển dữ liệu; kiểm thử, đào tạo, go-live. Mốc thời gian tại đây chỉ để khách hàng hình dung; thời hạn ràng buộc, ngày hoàn thành và cơ chế gia hạn áp dụng theo Hợp đồng và các mốc được hai bên xác nhận.'
        },
    ]
}

/** Default scope for design, media production and printed collateral. */
export const DEFAULT_DESIGN_PRINT_CONTRACT_APPENDIX: AppendixSection[] = [
    {
        key: 'scope_of_work',
        label: 'Phạm vi thiết kế và sản xuất',
        content: `Bên B thực hiện các hạng mục thiết kế, sản xuất nội dung và/hoặc in ấn được liệt kê cụ thể tại Bảng giá. Mỗi hạng mục chỉ bao gồm quy cách, số lượng, định dạng và tiêu chuẩn đã ghi nhận.`
    },
    {
        key: 'creative_process',
        label: 'Quy trình sáng tạo và duyệt mẫu',
        content: `Bên B gửi bản thiết kế hoặc bản xem thử theo từng hạng mục để Bên A duyệt qua email hoặc văn bản. Mỗi vòng chỉnh sửa là một tập hợp góp ý trong một email/tài liệu tổng hợp; thay đổi concept, quy cách, số lượng hoặc yêu cầu ngoài Bảng giá được xử lý như phát sinh.`
    },
    {
        key: 'deliverables',
        label: 'Sản phẩm bàn giao',
        content: `Bên B bàn giao file thiết kế, ấn phẩm thành phẩm, video/hình ảnh hoặc tài liệu liên quan theo đúng hạng mục đã chốt. File nguồn chỉ được bàn giao khi Bảng giá hoặc thỏa thuận riêng có quy định.`
    },
    {
        key: 'production_notes',
        label: 'Lưu ý sản xuất và in ấn',
        content: `Màu sắc hiển thị trên màn hình có thể khác màu thành phẩm do thiết bị, vật liệu và kỹ thuật in. Bên A chịu trách nhiệm xác nhận nội dung, chính tả và file duyệt trước khi Bên B sản xuất hàng loạt.`
    },
]

export function getDefaultDesignPrintContractAppendix(): AppendixSection[] {
    return DEFAULT_DESIGN_PRINT_CONTRACT_APPENDIX.map(section => ({ ...section }))
}
