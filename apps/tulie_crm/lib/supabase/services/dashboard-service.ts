...
export async function getCRMAlerts(): Promise<CRMAlert[]> {
    try {
        ...
        idleLeads?.forEach(lead => alerts.push({
            id: `lead_${lead.id}`,
            type: 'idle_lead',
            title: 'Lead mới chưa tương tác',
            description: `${lead.company_name || lead.full_name}`,
            severity: 'warning',
            link: `/leads`,
        ...
        // 3. Pending Quotes (> 5 ngày sent)
        const fiveDaysAgo = new Date(now.getTime() - 5 * 24 * 60 * 60 * 1000).toISOString()
        const { data: pendingQuotes } = await supabase
            .from('quotations')
            .select('id, quotation_number, title, created_at')
            .in('status', ['sent', 'viewed'])
            .lt('created_at', fiveDaysAgo)

        pendingQuotes?.forEach(quote => alerts.push({
            id: `quote_${quote.id}`,
            type: 'pending_quotation',
            title: 'Báo giá chưa chốt',
            description: `${quote.quotation_number || ''} ${quote.title || ''}`,
            severity: 'warning',
        ...
        const { data: overdueInvoices } = await supabase
            ...
            .lt('due_date', todayAtMidnight.toISOString())

        overdueInvoices?.forEach(invoice => alerts.push({
            id: `inv_${invoice.id}`,
        ...
        console.error('Error fetching CRM alerts:', err)
        return []
    }
}
