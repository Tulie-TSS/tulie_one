...
const getEnv = (key) => {
    ...
    if (!match) {
        const match2 = env.match(new RegExp(`${key}=([^\n]+)`))
...
const eventData = {
    ...
    brand_name: 'ISME\nCareer Fair 2026',
    logo_url: null,
    deadline_time: null, // Set khi biết ngày sự kiện cụ thể
    services: [
        {
            id: 'photo_id_1',
            name: 'Ảnh thẻ',
...
console.log(`\n📌 Truy cập: http://localhost:3001/event-sale (đã thêm localhost vào subdomains)`)
