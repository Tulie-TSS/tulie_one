...
export type KPICategory = "sales" | "marketing" | "operations" | "finance" | "lab" | "agency";
...
export interface OKRPeriod {
  ...
  organization_id: string;
  name: string;
  period_type: "quarter" | "year" | "month";
  start_date: string;
  end_date: string;
  is_active: boolean;
  created_at: string;
...
export interface KPIProgressLog {
  id: string;
  kpi_id: string;
  value: number;
  note: string | null;
...
export const OKR_STATUS_LABELS: Record<OKRStatus, { vi: string; en: string }> = {
  draft: { vi: "Nháp", en: "Draft" },
...
export const KPI_CATEGORY_LABELS: Record<KPICategory, { vi: string; en: string }> = {
  ...
  lab: { vi: "Lab", en: "Lab" },
  agency: { vi: "Agency", en: "Agency" },
};

export const KPI_FREQUENCY_LABELS: Record<KPIFrequency, { vi: string; en: string }> = {
...
export const METRIC_TYPE_LABELS: Record<MetricType, { vi: string; en: string }> = {
  number: { vi: "Số", en: "Number" },
  percentage: { vi: "Phần trăm", en: "Percentage" },
  currency: { vi: "Tiền tệ", en: "Currency" },
  boolean: { vi: "Có/Không", en: "Yes/No" },
};
